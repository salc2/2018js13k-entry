...
export const Camera = (props: CameraProps) => {
    ...
    const [cam, [player], [gra, walk, jum]]:Model = props.st;
    ...
    const [cx,cy,cw,ch,cvx,cvy] = cam;
     ...
     function onClickCanvas(event: MouseEvent){
        ...
        props.onChange({kind:"parameter", state: [cam, [[nx,ny,pw,ph,pVx,pVy,dir,onflo]], [gra, walk, jum]]})
    }

   return <form>
   <fieldset>
   <legend>Camera:</legend>
   Camera X: {cx}<input type="range" name="Camera X" value={cx} min="0" max="1000" onChange={ e => props.onChange({kind: "parameter", state:[[parseFloat(e.target.value),cy,cw,ch,cvx,cvy], [player], [gra, walk, jum]]})}/>
   <br/>
    Camera Y: {cy}<input type="range" name="Camera Y" value={cy} min="0" max="1000" onChange={ e => props.onChange({kind: "parameter", state:[[cx,parseFloat(e.target.value),cw,ch,cvx,cvy], [player], [gra, walk, jum]]})} />
    </fieldset>
    <fieldset>
    ...
    <br/>
    Gravity Speed:[{gra}] <input type="text" onChange={e => props.onChange( {kind:"parameter", state:[cam, [player], [parseFloat(e.target.value), walk, jum]]} )}/>
    <br/>
    Walk Speed:[{walk}] <input type="text" onChange={e => props.onChange( {kind:"parameter", state:[cam, [player], [gra, parseFloat(e.target.value), jum]]} )}/>
    <br/>
    Jump Speed:[{jum}] <input type="text" onChange={e => props.onChange( {kind:"parameter", state:[cam, [player], [gra, walk, parseFloat(e.target.value)]]} )}/>
...
export const updateDebug:Update<Action,Model> = (a: Action, m: Model) =>{
      const [cam, [player],pmt] = m;

switch (a.kind) {
    case "parameter":
    const [cam, [player], pmtr] = a.state;
    const moved:spacing = moveCamera(m[0],cam[0],cam[1],1000,1000);
    return [[moved, [player], pmtr],emptyCmd<Action>()];
    default:
      return update(a,m);
  }
}
